...
For this one-step problem, the expected-reward objective is
\[
\perf(\policy,\params)
=
\E_{\obs\sim P(\obs),\;\action\sim\policy(\cdot\mid\obs;\params)}
\left[\reward(\action,\obs)\right].
\]
The log-derivative trick gives
\[
\nabla_{\params}\perf(\policy,\params)
=
\E\left[
\reward(\action,\obs)
\nabla_{\params}\log \policy(\action\mid\obs;\params)
\right].
\]
In the code we use a mini-batch estimate with an advantage \advantage. The reference policy supplies a sampled KL penalty,
\[
\shapedreward_i
=
\reward_i
...
-
\log \refpolicy(\action_i\mid\obs_i)
\right),
\]
and the standardized advantage is
\[
\advantage_i
=
\frac{\shapedreward_i-\operatorname{mean}(\shapedreward)}
{\operatorname{std}(\shapedreward)+\epsilon}.
\]

Once the batch
\[
\{(\obs_i,\action_i,\advantage_i)\}_{i=1}^{N}
\]
has been sampled, the observations \(\obs_i\), sampled actions \(\action_i\), and advantage estimates \(\advantage_i\) are treated as fixed data for the gradient step. The only quantity in the surrogate loss that depends on the current parameters \(\params\) is the log-probability
\[
\log \policy(\action_i\mid \obs_i;\params).
\]
Implementations typically use a policy-gradient surrogate, written with a stop-gradient on the advantages:
\[
\loss_{\mathrm{PG}}(\params)
=
-
\frac{1}{N}
\sum_{i=1}^{N}
...
\log \policy(\action_i\mid\obs_i;\params),
\]
where \(\operatorname{sg}[\cdot]\) means ``treat this quantity as a constant when differentiating.''

Now differentiate term by term. By linearity of the gradient,
\[
...
\log \policy(\action_i\mid\obs_i;\params).
\]
Substituting this back into the batch loss gives the correct policy gradient:
\[
\nabla_{\params}\loss_{\mathrm{PG}}(\params)
=
-
...
In the code that follows, this is why the policy-gradient loss is usually implemented as
\[
\texttt{loss = -(advantages.detach() * log\_probs).mean()}.
\]
The call to \texttt{detach()} enforces the mathematical convention above: the advantage is used as a scalar weight on the log-probability, not as something through which the optimiser should backpropagate during the policy update.
...
def policy_gradient_step(model, ref_model, opt, batch_size=64, beta=0.02):
    ...
    # 3. Evaluate the same actions under the frozen SFT/reference policy.
    with torch.no_grad():
        ref_logits = ref_model(x, lengths)
        ref_dist = torch.distributions.Categorical(logits=ref_logits)
        ref_logp = ref_dist.log_prob(actions)

        reward = (actions == targets).float()
        sampled_kl = logp - ref_logp
        shaped_reward = reward - beta * sampled_kl
        advantage = (shaped_reward - shaped_reward.mean()) / (
            shaped_reward.std() + 1e-8
        )

    # 4. REINFORCE: gradient of -A log pi is -A grad log pi.
